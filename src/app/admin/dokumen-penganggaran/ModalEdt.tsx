...
function ModalEditDokpera(post: Data) {
  const [modal, setModal] = useState(false);
  const [judul, setTitle] = useState<string>(post.judul);
  const [dokumen, setDokmumen] = useState<string>(post.dokumen);
  ...
  const handleDokumenChange = (event: ChangeEvent<HTMLSelectElement>) => {
    setDokmumen(event.target.value);
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!judul || !dokumen || !file) {
      alert("Data belum disi semua");
      return;
    }
    ...
    try {
      ...
      const response = await axios.patch(
        `https://lab.dima-dan-rendy.shop/dokanggaran/${post.id}`,
        formData,
        {
          headers: {
          ...
          <div className="w-full max-w-xs">
            <form
              ...
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">
                  Dokumen
                  <select className="select w-full select-bordered select-sm max-w-xs" value={dokumen} onChange={handleDokumenChange} >
                    <option value="apbd">APBD</option>
                    <option value="kua">KUA</option>
                    <option value="ppas">PPAS</option>
                    <option value="notakeuangan">NOTA KEUANGAN</option>
                    <option value="dpa">DPA</option>
                    <option value="dppa">DPPA</option>
                    <option value="rka">RKA</option>
                    <option value="beritaacara">BERITA ACARA KESEPAKATAN</option>
...
export default ModalEditDokpera;
