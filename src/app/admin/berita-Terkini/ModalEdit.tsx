...
function EditBeritaTerkini(post: Ipost) {
  const [modal, setModal] = useState(false);
  const [judul, setJudul] = useState<string>(post.judul);
  const [descrip, setDescrip] = useState<string>(post.descrip);
  const [tanggal, setTanggal] = useState<string>(post.tanggal);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://lab.dima-dan-rendy.shop/berkin`,
        formData,
        {
          headers: {
...
export default EditBeritaTerkini;
